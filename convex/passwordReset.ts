...
/**
 * Password-reset OTP provider. Convex Auth's generic Email provider with
 * our own sender (Postmark via lib/email), so no third-party auth
 * provider package is involved.
 */
export const PostmarkOTPPasswordReset = Email({
  id: "postmark-otp",
  maxAge: 60 * 60, // seconds: the code works for one hour
  ...
  async sendVerificationRequest({ identifier: email, token }) {
    ...
    const html = emailLayout({
      ...
      bodyHtml: [
        ...
        `</div>`,
        `<p style="${emailStyles.paragraph}">Enter it on the reset page along with your new password. The code works for one hour.</p>`,
